/**
 * Ed25519 sign — zero external dependencies.
 * Field arithmetic copied verbatim from TweetNaCl.js (public domain).
 * SHA-512 delegated to browser's crypto.subtle.digest (available since 2014).
 *
 * Exports:
 *   generateKeyPair()           → { publicKey: Uint8Array(32), secretKey: Uint8Array(64) }
 *   generateKeyPairFromSeed(s)  → same, deterministic from 32-byte seed
 *   sign(message, secretKey)    → Uint8Array(64) signature
 */
...
// ─── GF(2^255-19) helpers (verbatim from TweetNaCl.js) ───────────────────────
function gf(init) {
    var r = new Float64Array(16);
    if (init) for (var i = 0; i < init.length; i++) r[i] = init[i] | 0;
    return r;
}
...
function M(o, a, b) {
    ...
    v = a[0]; t0 += v * b0; t1 += v * b1; t2 += v * b2; t3 += v * b3; t4 += v * b4; t5 += v * b5; t6 += v * b6; t7 += v * b7; t8 += v * b8; t9 += v * b9; t10 += v * b10; t11 += v * b11; t12 += v * b12; t13 += v * b13; t14 += v * b14; t15 += v * b15;
    ...
    t0 += 38 * t16; t1 += 38 * t17; t2 += 38 * t18; t3 += 38 * t19; t4 += 38 * t20; t5 += 38 * t21;
    t6 += 38 * t22; t7 += 38 * t23; t8 += 38 * t24; t9 += 38 * t25; t10 += 38 * t26; t11 += 38 * t27;
    t12 += 38 * t28; t13 += 38 * t29; t14 += 38 * t30;

    c = 1; v = t0 + c + 65535; c = Math.floor(v / 65536); t0 = v - c * 65536;
...
export async function generateKeyPair() {
    return generateKeyPairFromSeed(crypto.getRandomValues(new Uint8Array(32)));
}
 ...
 * @param {Uint8Array} secretKey  64-byte key (seed || pubkey)
 * @returns {Promise<Uint8Array>} 64-byte signature
 */
export async function sign(message, secretKey) {
    var d, r, h, x = new Float64Array(64);
    var p = [gf(), gf(), gf(), gf()];
    var mlen = message.length;

    d = await sha512(secretKey.subarray(0, 32));
    d[0] &= 248; d[31] &= 127; d[31] |= 64;

    // nonce = sha512(d[32..63] || message)
    var nonceData = new Uint8Array(32 + mlen);
    for (var i = 0; i < 32; i++) nonceData[i] = d[32 + i];
    for (i = 0; i < mlen; i++) nonceData[32 + i] = message[i];
    r = await sha512(nonceData);

    // nonce scalar mod L
    var rScalar = new Float64Array(64);
    for (i = 0; i < 64; i++) rScalar[i] = r[i];
    var nonce = new Uint8Array(32);
    modL(nonce, rScalar);

    // R = nonce * B
    scalarbase(p, nonce);
    var sigR = new Uint8Array(32);
    pack(sigR, p);

    // H = sha512(R || pk || m)
    var hData = new Uint8Array(64 + mlen);
    for (i = 0; i < 32; i++) hData[i] = sigR[i];
    for (i = 0; i < 32; i++) hData[32 + i] = secretKey[32 + i];
    for (i = 0; i < mlen; i++) hData[64 + i] = message[i];
    h = await sha512(hData);

    // S = (nonce + H * a) mod L
    for (i = 0; i < 64; i++) x[i] = 0;
    for (i = 0; i < 32; i++) x[i] = nonce[i];
    for (i = 0; i < 32; i++) for (var j = 0; j < 32; j++) x[i + j] += h[i] * d[j];

    var sig = new Uint8Array(64);
    var sigS = new Uint8Array(32);
    modL(sigS, x);
    for (i = 0; i < 32; i++) sig[i] = sigR[i];
    for (i = 0; i < 32; i++) sig[32 + i] = sigS[i];
